...
class FeedController extends Controller
{
    ...
    public function index(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            'feed:v4:index:'.($userId ?? 'guest').':'.md5(json_encode($request->query())),
            120,
    ...
    public function personal(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            "feed:v4:personal:{$userId}:".md5(json_encode($request->query())),
            120,
    ...
    public function trending(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            'feed:v4:trending:'.($userId ?? 'guest').':'.md5(json_encode($request->query())),
            120,
    ...
    public function userPlants(FeedQueryRequest $request, $userId)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            'feed:v4:user:'.($currentUserId ?? 'guest').":{$userId}:".md5(json_encode($request->query())),
            120,
    ...
    public function withTips(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            'feed:v4:with_tips:'.($userId ?? 'guest').':'.md5(json_encode($request->query())),
            120,
    ...
    public function recommendations(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            "feed:v4:recommendations:{$userId}:".md5(json_encode($request->query())),
            120,
    ...
    public function likedPlants(FeedQueryRequest $request)
    {
        ...
        $payload = $this->cache->remember(
            ['feed'],
            "feed:v4:liked:{$userId}:".md5(json_encode($request->query())),
            120,
