...
const isPlantReportsDialogOpen = ref(false);
const plantReportsLoading = ref(false);
const plantReports = ref([]);
...
const canModeratePlantDirectly = computed(
    () => authStore.isAdmin && !isOwnPlant.value,
);
const canUsePlantFlag = computed(
    () => canReportPlant.value || canModeratePlantDirectly.value,
);
...
const canViewPlantReports = computed(
    () => authStore.isAuthenticated && (isOwnPlant.value || authStore.isAdmin),
);
const plantReportSummary = computed(() => plant.value?.reportSummary || {});
const plantReportBadge = computed(() =>
    plantReportIndicator(plantReportSummary.value),
);
const ownerWarningsCount = computed(
    () =>
        Number(
            plant.value?.raw?.owner?.warnings_count ??
                plant.value?.raw?.user?.warnings_count ??
                plant.value?.raw?.user?.data?.warnings_count ??
                0,
        ) || 0,
);
...
const editCareValue = (type, fallbackDays) => {
    const setting =
        plant.value?.careSettings?.[type] || plant.value?.care?.[type];
...
const openReportDialog = () => {
    if (canModeratePlantDirectly.value) {
        moderationAction.value = "warn_user";
        moderationComment.value = "";
        isModerationDialogOpen.value = true;
        return;
    }
...
const loadPlantReports = async () => {
    if (!canViewPlantReports.value) return;

    plantReportsLoading.value = true;
    try {
        const payload = await apiClient.get(
            `/plants/${plantApiId.value}/reports?per_page=50`,
        );
        plantReports.value = payload?.data?.data || payload?.data || [];
    } catch (error) {
        toast.error(error.message);
    } finally {
        plantReportsLoading.value = false;
    }
};
...
const openPlantReportsDialog = async () => {
    if (!canViewPlantReports.value) return;

    isPlantReportsDialogOpen.value = true;
    await loadPlantReports();
};

const closePlantReportsDialog = () => {
    isPlantReportsDialogOpen.value = false;
};
    ...
    try {
        ...
        if (rejectTipReport.value) {
            await socialStore.reportTip(
                tip.id,
                "other",
                rejectTipReportDetails.value.trim(),
            );
        }
        toast.success(
            rejectTipReport.value
                ? "Совет отклонён, жалоба отправлена"
                : "Совет отклонён",
        );
...
const saveEditDialog = handleEditSubmit(async (values) => {
    ...
    try {
        await plantStore.updatePlantDetails(plant.value, values, {
            reload: false,
        });
        await plantStore.updatePlantCare(plant.value, values, {
            reload: false,
        });
...
const deleteActivePhoto = async () => {
    ...
    try {
        await plantStore.deletePlantImage(
            activeImage.value.id,
            plantApiId.value,
        );
        plantImages.value = await plantStore.loadPlantImages(plantApiId.value);
...
<template>
    ...
    >
                ...
                >
                    ...
                    >
                        <span
                            ...
                            :style="{
                                backgroundColor:
                                    careTypes[item.type]?.color || '#7ea885',
                            }"
                        >
        ...
        >
            ...
            <section
                v-if="canCompleteCareForPlant"
                class="panel plant-tasks-panel"
            >
            ...
            >
                ...
                <div
                    ...
                    class="tips-list owner-tips-list"
                >
                    <article
                ...
                <div
                    v-else-if="!isOwnPlant && !canSuggestForPlant"
                    class="guest-tips-empty"
                >
                    <strong>Советов пока нет</strong>
                    <p class="muted">
                        Станьте первым, кто оставит практическую рекомендацию.
                    </p>
                </div>
            </section>
        </div>
    ...
    >
        <section class="edit-dialog__card reject-tip-dialog">
            ...
            <div class="edit-dialog__actions reject-tip-dialog__actions">
                <UiButton
                    type="button"
                    class="reject-tip-dialog__submit"
    ...
    <div
        v-if="isPlantReportsDialogOpen"
        class="report-dialog"
        @click.self="closePlantReportsDialog"
    >
        <section class="report-dialog__card report-dialog__card--wide">
            <header class="report-dialog__head">
                <h2 class="panel__title">Жалобы на растение</h2>
                <button
                    type="button"
                    class="report-dialog__close"
                    aria-label="Закрыть"
    ...
    <div
        v-if="isModerationDialogOpen"
        class="report-dialog"
        @click.self="closeModerationDialog"
    >
        <form
            class="report-dialog__card"
            @submit.prevent="submitPlantModeration"
        >
            <header class="report-dialog__head">
                <h2 class="panel__title">Модерация растения</h2>
                <button
                    type="button"
                    class="report-dialog__close"
                    aria-label="Закрыть"
            ...
            <div class="report-dialog__form">
                <label class="moderation-option">
                    ...
                    />
                    <span>
                        <strong>Предупредить и скрыть растение</strong>
                        <small
                            >Растение временно исчезнет из паблика, владелец
                            сможет вернуть его позже.</small
                        >
                    </span>
                </label>
                <label class="moderation-option">
                ...
                <label class="moderation-option">
                    ...
                    <span>
                        ...
                        <small
                            >Публичные растения будут скрыты, советы удалены,
                            ранг обнулён.</small
                        >
                    </span>
                </label>
                <p
                    v-if="selectedModerationIsFinalWarning"
...
.plant-report-summary[data-tone="danger"] {
    color: #8f1f10;
    background: #ffd8d2;
}
...
.moderation-option span {
    display: grid;
    gap: 4px;
}

.moderation-option small,
.resolution-warning {
    color: var(--color-muted);
    font-weight: 800;
}
