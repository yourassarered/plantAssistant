...
const isPublicLocked = computed(() =>
    Boolean(editingPlant.value?.isPublicLocked),
);
...
const createFormSubtitle = computed(
    () => "Новая запись будет добавлена в ваш список растений.",
);
...
const onSubmit = handleSubmit(async (values) => {
    try {
        if (isEditMode.value && editingPlant.value) {
            ...
            await plantStore.addPlant(values, photoFile.value);
            photoFile.value = null;
    ...
    <section class="page">
        ...
        <nav v-if="isEditMode && editingPlant" class="edit-actions">
            ...
            <RouterLink :to="`/plants/${editingPlant.id}/photos`">
                <UiButton
                    :variant="pageMode === 'photos' ? 'primary' : 'ghost'"
                >
                    <Camera :size="16" /> Фото
                </UiButton>
            </RouterLink>
        ...
        <div v-if="!authStore.isAuthenticated" class="panel plant-form-auth">
            Нужно войти в профиле, чтобы управлять растениями и расписанием
            ухода.
        ...
        >
            <form class="plant-form" @submit.prevent="onSubmit">
                ...
                <div class="plant-form__body">
                    <section
                        v-if="showPlantSettings"
                    ...
                    >
                        ...
                        <div class="create-photo-upload">
                            ...
                            <p>
                                Фото загрузится после создания растения. Если
                                оставить поле пустым, будет показан плейсхолдер.
                            </p>
                        </div>
                    </section>
                    ...
                    <section v-if="showCareSettings" class="panel care-editor">
                        ...
                        <div class="care-grid">
                            <article
                                ...
                                class="care-card"
                                :class="{
                                    'care-card--off': !row.enabled.value,
                                }"
                            >
...
.plant-form-shell--modal .plant-form__body {
    display: grid;
    gap: 12px;
    min-height: 0;
    overflow-y: auto;
...
.plant-form-modal-head .page-title,
.plant-form-modal-head .page-subtitle {
    margin: 0;
}
...
.plant-form-shell--modal .form-section,
.plant-form-shell--modal .care-editor {
    padding: 14px;
    border-radius: var(--radius-sm);
    background: #f8faf5;
    box-shadow: none;
